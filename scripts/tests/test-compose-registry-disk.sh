...
check(len(COMPOSE_REGISTRY) == 89, f"registry has 89 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 90, f"disk has 90 compose files (got {len(disk_paths)})")
