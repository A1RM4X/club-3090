...
check(len(COMPOSE_REGISTRY) == 137, f"registry expects 137 entries (got {len(COMPOSE_REGISTRY)})")
check(len(disk_paths) == 140, f"disk expects 140 compose files (got {len(disk_paths)})")
