...
COMPOSE_REGISTRY = {
    ...
    "vllm/qwen38-27b-multi4-dflash2": _entry(
        ...
        tp=4, max_ctx=262144, max_num_seqs=2, mem_util=0.85,
        compose_path="models/qwen3.8-27b/vllm/compose/multi4/autoround-int4/dflash2.yml",
        default_port=8098,
        kvcalc_key="SKIP",
        act_format="int8", act8_capable=True,
        status="experimental",
        status_note="Qwen3.8-27B FAST tier, 4-card (TP=4), DFlash2 drafter (incoai/Qwen3.8-27B-DFlash2, n=7), fp8 KV @262144, gmu 0.85. ⛔ 4-CARD IS COMMUNITY-VALIDATED BY DESIGN and PERMANENT: the maintainer rig has two 3090s, so TP=4 can never boot here. ⚠️ THE DFlash2 DRAFTER IS NOT A DROP-IN: its candidate selector only runs in the V2 speculator, so this compose mounts the vllm-dflash2 overlay (the 6-piece port of vllm-project/vllm#52816) which installs the model + DFlash2Speculator + registry/runner wiring at boot and hard-fails on anchor drift. On the V1 runner the same checkpoint silently degrades to base DFlash and weight-load fails with 'no module named candidate_selector'. ⚠️ THE DRAFTER IS HEAVY: the codebooks (~0.5 GiB/GPU) + 5-layer backbone eat the headroom a large-context KV cache leaves, so gmu ships at 0.85 (not the MTP sibling's 0.95) and max-model-len at 262144 (not 512K) -- a 512K/0.90 config left ~0.5 GiB free and the first ~8K prefill OOM'd the engine (exit 137). 256K/0.85 gave 1,014,705 KV tokens with ~1.8 GiB free and survived. MEMPROBE=1 logs the per-step peak transient activation so you can size gmu to the real need before pushing context higher. ⚠️ NOT YET BENCHED on this rig -- the first community boot IS the validation; report via numbers-from-your-rig. Detail: models/qwen3.8-27b/vllm/patches/vllm-dflash2/install.sh + the compose header.",
    ),
