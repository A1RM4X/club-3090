...
COMPOSE_REGISTRY = {
    ...
    "vllm/qwen38-27b-multi4-dflash2": _entry(
        model="qwen3.8-27b", weights_variant="autoround-int4", workload="long-ctx-single", chat_template="native",
        engine="vllm-stable", drafter="incoai-qwen38-dflash2", kv_format="fp8_e4m3",
        tp=4, max_ctx=262144, max_num_seqs=2, mem_util=0.80,
        compose_path="models/qwen3.8-27b/vllm/compose/multi4/autoround-int4/dflash2.yml",
        default_port=8098,
        kvcalc_key="SKIP",
        act_format="int8", act8_capable=True,
        status="experimental",
        status_note="Qwen3.8-27B FAST tier, 4-card (TP=4), DFlash2 drafter (incoai/Qwen3.8-27B-DFlash2, n=7), fp8 KV, gmu 0.80, max-model-len 262144 (the model's NATIVE ceiling). ⛔ 4-CARD IS COMMUNITY-VALIDATED BY DESIGN and PERMANENT: the maintainer rig has two 3090s, so TP=4 can never boot here. ⚠️ THE DFlash2 DRAFTER IS NOT A DROP-IN: its candidate selector only runs in the V2 speculator, so this compose mounts the vllm-dflash2 overlay (the 6-piece port of vllm-project/vllm#52816) which installs the model + DFlash2Speculator + registry/runner wiring at boot and hard-fails on anchor drift. On the V1 runner the same checkpoint silently degrades to base DFlash and weight-load fails with 'no module named candidate_selector'. ⚠️ THE DRAFTER IS HEAVY: the codebooks (~0.5 GiB/GPU) + 5-layer backbone eat the headroom a large-context KV cache leaves; 512K/0.90 OOM'd the first ~8K prefill (exit 137), 0.80 is the working gmu. ⚠️ 512K IS NOT A REAL SINGLE-SEQUENCE CEILING: the KV pool + vLLM mamba-hybrid scheduler cap a single sequence at ~240K (91% of 256K); the verify-stress ceiling ladder filled cleanly to 240,635 tok with 0 MiB growth, so this ships 262144 (the native max_position_embeddings), not 524288 — ~920K-token pool holds ~3.5 full 256K sequences. DFlash2 IS working: measured acceptance length 4.68/7.20/6.85 (53-91% draft acceptance) under active generation — well above the MTP sibling's 2.62 (the bench.sh 'AL 2.00/14.3%' reading is an idle-window artifact; the drafter only reports a real AL under load). FULL GATE GREEN (report.sh --full, 4x3090 Turbo 220W cap): verify-full 9/9 (MTP-AL 4.40) · verify-stress 7/7 (ceiling ladder 6/6 to 240,635 tok, 0 MiB growth) · soak-continuous PASS (0 MiB growth, 0/25 silent-empty, 100% retention, p50 decode 280) · bench narrative 97.9 / code 190.2 decode TPS, prefill 1869@10K/1612@90K, 0 MiB leak. Interconnect: 2x NVLink pairs (0,2)/(1,3) + PCIe P2P, custom-AR auto-off (expected, #786), not the bottleneck (PCIe decode peak 39% of link). 8-pack quality (benchlocal v0.9.9): no-thinking 120/150 pass@1 (80%) / 124/150 pass@3 (83%); thinking-on 128/150 (85%) / 135/150 (90%) — a +8pp/+7pp thinking lift concentrated in the reasoning-heavy packs (instructfollow 14->15, reasonmath 11->13, bugfind 12->14, hermesagent 13->15, cli-40 29->30); soft packs cli-40/hermesagent are the known Qwen-family floors, structoutput a clean 15/15 in both arms. ALL 8 GATES GREEN. Detail: models/qwen3.8-27b/vllm/patches/vllm-dflash2/install.sh + the compose header.",
    ),
