...
COMPOSE_REGISTRY = {
    ...
    "vllm/qwen38-27b-multi4-dflash2": _entry(
        ...
        act_format="int8", act8_capable=True,
        status="experimental",
        status_note="Qwen3.8-27B FAST tier, 4-card (TP=4), DFlash2 drafter (incoai/Qwen3.8-27B-DFlash2, n=7), fp8 KV, gmu 0.80, max-model-len 262144 (the model's NATIVE ceiling). ⛔ 4-CARD IS COMMUNITY-VALIDATED BY DESIGN and PERMANENT: the maintainer rig has two 3090s, so TP=4 can never boot here. ⚠️ THE DFlash2 DRAFTER IS NOT A DROP-IN: its candidate selector only runs in the V2 speculator, so this compose mounts the vllm-dflash2 overlay (the 6-piece port of vllm-project/vllm#52816) which installs the model + DFlash2Speculator + registry/runner wiring at boot and hard-fails on anchor drift. On the V1 runner the same checkpoint silently degrades to base DFlash and weight-load fails with 'no module named candidate_selector'. ⚠️ THE DRAFTER IS HEAVY: the codebooks (~0.5 GiB/GPU) + 5-layer backbone eat the headroom a large-context KV cache leaves; 512K/0.90 OOM'd the first ~8K prefill (exit 137), 0.80 is the working gmu. ⚠️ 512K IS NOT A REAL SINGLE-SEQUENCE CEILING: the KV pool + vLLM mamba-hybrid scheduler drop any single sequence past ~244K (the verify-stress ceiling ladder passed ~244,470 tok and the 275K rung was DROPPED with a CUDA device-side assert in the mamba-hybrid/FlashInfer path, then auto-restart — a vLLM robustness wart, not a port bug). So this ships 262144, not 524288; ~1.05M-token pool holds ~4 full 256K sequences. DFlash2 IS working: measured acceptance length 3.03 / 29% draft acceptance (the bench.sh 'AL 2.00/14.3%' reading was an idle-window artifact — the drafter only reports a real AL under active generation). ⚠️ NOT YET BENCHED for TPS on this rig — prefill-stability + AL are measured, the TPS/decode-quality numbers are the open item; report via numbers-from-your-rig. Detail: models/qwen3.8-27b/vllm/patches/vllm-dflash2/install.sh + the compose header.",
    ),
