...
COMPOSE_REGISTRY = {
    ...
    "vllm/qwen38-27b-multi4-dflash2": _entry(
        ...
        act_format="int8", act8_capable=True,
        status="experimental",
        status_note="Qwen3.8-27B FAST tier, 4-card (TP=4), DFlash2 drafter (incoai/Qwen3.8-27B-DFlash2, n=7), fp8 KV, gmu 0.80. ⛔ 4-CARD IS COMMUNITY-VALIDATED BY DESIGN and PERMANENT: the maintainer rig has two 3090s, so TP=4 can never boot here. ⚠️ THE DFlash2 DRAFTER IS NOT A DROP-IN: its candidate selector only runs in the V2 speculator, so this compose mounts the vllm-dflash2 overlay (the 6-piece port of vllm-project/vllm#52816) which installs the model + DFlash2Speculator + registry/runner wiring at boot and hard-fails on anchor drift. On the V1 runner the same checkpoint silently degrades to base DFlash and weight-load fails with 'no module named candidate_selector'. ⚠️ THE DRAFTER IS HEAVY: the codebooks (~0.5 GiB/GPU) + 5-layer backbone eat the headroom a large-context KV cache leaves. The compose ships MAX_MODEL_LEN=524288 (a YaRN-free position-embedding extension beyond the model's native 262144 — the registry entry stays at 262144, the model's declared ceiling, so C11 holds) with gmu 0.80. VALIDATED 2026-08-19 on a 4x3090 rig: that 512K/0.80 config survived the full prefill ladder (32K/65K/130K/260K tokens) with no OOM — 1,049,336 KV tokens (10.85 GiB/GPU), ~3 GiB free per card, ~2 concurrent 512K sequences. The 512K/0.90 config OOM'd the first ~8K prefill (exit 137), so 0.80 is the working gmu at 512K. If you want the model's native 262144 (more concurrency, ~4 sequences), set MAX_MODEL_LEN=262144 and raise gmu toward 0.85. MEMPROBE=1 logs the per-step peak transient activation so you can size gmu to the real need. ⚠️ NOT YET BENCHED for acceptance-rate/TPS on this rig -- the prefill-stability is validated, the decode-quality numbers are the open item; report via numbers-from-your-rig. Detail: models/qwen3.8-27b/vllm/patches/vllm-dflash2/install.sh + the compose header.",
    ),
