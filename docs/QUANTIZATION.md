...
| `fp8_e4m3` | 8 | vLLM | Same bytes as e5m2; **better numeric precision** (more mantissa). Availability depends on the model/loader/backend route, not SM alone. Verified on Ampere sm_86 through FlashInfer with Qwen-AgentWorld AWQ on stock vLLM v0.25.1; other non-FP8-weight models can still route to Triton and reject sm_86. ⚠️ **Storage-only on consumer GPUs** — native FP8 *attention* compute needs FA3 (Hopper) or trtllm-gen FMHA (datacenter Blackwell). Carried directly by the composes; `KV_CACHE_DTYPE=` overrides. (It was launcher-injected on sm_89+ under [#246](https://github.com/noonghunna/club-3090/issues/246) Phase 1 — retired in [#1371](https://github.com/noonghunna/club-3090/issues/1371) once every slug shipped e4m3 and the injector had nothing left to upgrade.) Full arch matrix: [DTYPE_MATRIX](DTYPE_MATRIX.md). |
