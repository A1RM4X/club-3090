...
> ⚠️ **The variable has to reach the container, and no shipped compose declares it yet** (0 of 53 vLLM composes as of 2026-08-04 — [#873](https://github.com/noonghunna/club-3090/issues/873) hit exactly this: the env var was set, the check never ran, and no Transfer-check line appeared). Exporting it in your shell or `.env` is **not** enough. Add the bare passthrough to the compose's `environment:` block alongside the ones already there (`CUDA_VISIBLE_DEVICES`, `VLLM_USE_DEEP_GEMM`), then relaunch:
>
> ```yaml
>     environment:
>       - VLLM_SKIP_P2P_CHECK
