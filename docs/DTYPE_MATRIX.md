...
**The launchers no longer inject `KV_CACHE_DTYPE` at all.** [#246](https://github.com/noonghunna/club-3090/issues/246) Phase 1 did, on sm_89+, for the pilot slugs `vllm/dual`/`vllm/minimal` — until those composes migrated to `fp8_e4m3` outright, which orphaned the injector's only source format and left it returning nothing on every card. Retired in [#1371](https://github.com/noonghunna/club-3090/issues/1371); the composes now carry the format directly and `KV_CACHE_DTYPE=` still overrides. **The reframe from the first cross-rig data stands, and is the reason the injection was never worth reviving:** e4m3 is *not* a throughput win on consumer cards (it's storage-only there, ≡ e5m2 — see above). It's still the right default on sm_89+ as the **better-precision** FP8 KV format at zero speed cost — but the #246 "e4m3 for native FP8 compute" hypothesis only holds on Hopper / datacenter Blackwell, which the community doesn't run.
...
- **Ampere support for fp8_e4m3 KV is route-conditional, not simply weights-conditional.** Qwen FP8 weights use FlashInfer (#594), while Gemma W4A16 routes to Triton and rejects sm_86 (`gemma-4-31b.md`, 2026-07-01). Qwen-AgentWorld adds a third measured case: its pure-INT4 AWQ compressed-tensors checkpoint routes FP8 KV to FlashInfer on stock vLLM v0.25.1 and passes full-context validation. `weights_variant` is therefore only a proxy; verify the selected backend in the boot log and complete a real request.
...
1. **Compressed-tensors does not determine FP8-KV support by itself.** Some AWQ / FP8-weight / INT8-weight checkpoints die with `ValueError: fp8_e5m2 kv-cache is not supported with fp8 checkpoints`, whether quantization is explicit or auto-detected. Stock vLLM v0.25.1 does not apply that rejection universally: Qwen-AgentWorld's pure-INT4 AWQ compressed-tensors checkpoint booted with `fp8`, `fp8_e4m3`, and `fp8_e5m2` on sm_86. E4M3 selected FlashInfer, allocated 1,795,289 KV tokens, passed exact recall through 240K, and sustained four simultaneous ~261.5K-token prompts. Treat INT8-PTH as the fallback when the exact model/image rejects FP8, not as a loader-wide rule.
...
| Qwen-AgentWorld AWQ compressed-tensors on stock v0.25.1 | `fp8` (`fp8_e4m3`) | none |
| compressed-tensors model/image that rejects FP8 | `int8_per_token_head` | none — **except Gemma-4: +#40391** |
